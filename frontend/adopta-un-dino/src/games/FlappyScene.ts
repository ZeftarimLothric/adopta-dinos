...
export default class FlappyScene extends Phaser.Scene {
  ...
  private effectsTimer!: Phaser.Time.TimerEvent;
  ...
  constructor() {
    ...
    this.CONFIG = {
      ...
      OBSTACLES: {
        ...
        GAP: this.isMobile ? Math.floor(this.gameHeight * 0.5) : 180, // Gap proporcional al tamaño
  ...
  create(): void {
    ...
    this.ptero.body.setGravityY(this.CONFIG.PTERO.GRAVITY);
    this.ptero.body.setMaxVelocity(400, this.CONFIG.PTERO.MAX_VELOCITY);
    
    // Establecer escala visible
    this.ptero.setScale(1);
    
    // Configurar hitbox
    this.ptero.body.setSize(25, 20);
  ...
  private checkPerfectTiming(): void {
    // Solo incrementar combo si realmente hay un obstáculo cerca
    let nearObstacle = false;
    
    this.pipes.children.entries.forEach(pipe => {
      const pipeSprite = pipe as any;
      const distance = Math.abs(pipeSprite.x - this.ptero.x);
      
      // Solo contar como "perfect" si está muy cerca del gap
      if (distance < 30 && pipeSprite.pipeType === 'top') {
        ...
        // Solo es "perfect" si está en el centro del gap
  ...
  private addPipes(): void {
    ...
    topPipe.body.setSize(this.CONFIG.OBSTACLES.WIDTH, topHeight);
    (topPipe as any).scored = false;
    (topPipe as any).pipeType = 'top';
    ...
    bottomPipe.body.setSize(this.CONFIG.OBSTACLES.WIDTH, bottomHeight);
    (bottomPipe as any).scored = false;
    (bottomPipe as any).pipeType = 'bottom';
  ...
  private collectPowerUp(ptero: Phaser.Physics.Arcade.Sprite, powerUp: Phaser.Physics.Arcade.Sprite): void {
    this.createCollectEffect(powerUp.x, powerUp.y);
    
    switch ((powerUp as any).powerType) {
      case 'powerup_slow':
      ...
      case 'powerup_points':
        ...
        // TAMBIÉN verificar DinoPoints con los puntos del power-up
        ...
        if (currentPackage > this.packagesGiven) {
          ...
          window.dispatchEvent(new CustomEvent('flappyPointsEarned', {
            detail: { 
              points: this.CONFIG.GAME.DINOPOINTS_AMOUNT, // Aquí debería dar 20
              score: this.score 
            }
          }));
        }
  ...
  private updateDifficulty(): void {
    ...
    if (newSpeed !== this.currentSpeed) {
      ...
      this.obstacleTimer.delay = this.currentSpawnDelay;
  ...
  private checkScoring(): void {
    this.pipes.children.entries.forEach(pipe => {
      ...
      // Solo procesar obstáculos tipo 'top' para evitar duplicados
      if (pipeSprite.pipeType === 'top' && !pipeSprite.scored && 
          pipeSprite.x + this.CONFIG.OBSTACLES.WIDTH < this.ptero.x) {
        
        // Marcar AMBOS obstáculos (top y bottom) como scored
        this.pipes.children.entries.forEach(otherPipe => {
          const otherSprite = otherPipe as any;
          if (Math.abs(otherSprite.x - pipeSprite.x) < 10) { // Misma columna
            otherSprite.scored = true;
          }
        });
        
        // Sumar punto base (SOLO 1 punto por obstáculo)
        this.score += this.CONFIG.GAME.POINTS_PER_OBSTACLE;
        
        // NO SUMAR EL COMBO AL SCORE AQUÍ
        // El combo es solo visual/feedback
        
        // Dispatch del evento de score
        window.dispatchEvent(new CustomEvent('flappyScoreUpdate', {
          detail: { score: this.score }
        }));
        
        // Verificar DinoPoints
        const currentPackage = Math.floor(this.score / this.CONFIG.GAME.DINOPOINTS_INTERVAL);
        if (currentPackage > this.packagesGiven) {
          this.packagesGiven = currentPackage;
  ...
  private handleCollision(ptero: Phaser.Physics.Arcade.Sprite, pipe: Phaser.Physics.Arcade.Sprite): void {
    if (this.isInvulnerable || this.gameOver) return;
    
    // Resetear combo al chocar
    this.combo = 0;
    
    this.createExplosion(ptero.x, ptero.y);
  ...
  public resetGame(): void {
    ...
    this.obstacleTimer.paused = true;
    this.powerUpTimer.paused = true;
    this.obstacleTimer.delay = this.CONFIG.OBSTACLES.SPAWN_DELAY;
  ...
  update(): void {
    ...
    // Rotación del pterosaurio
    const angle = Phaser.Math.Clamp(
      this.ptero.body.velocity.y * this.CONFIG.PTERO.ROTATION_FACTOR, 
      -30, 
      30
    );
