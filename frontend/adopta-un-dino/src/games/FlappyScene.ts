...
import pteroImage from "../assets/Ptero.png";
...
export default class FlappyScene extends Phaser.Scene {
  ...
  constructor() {
    ...
    console.log(
      `Pantalla: ${screenWidth}x${screenHeight}, Juego: ${this.gameWidth}x${this.gameHeight}, Móvil: ${this.isMobile}`
    );
  ...
  create(): void {
    ...
    if (pteroBody) {
      ...
      pteroBody.setSize(this.ptero.width * 0.8, this.ptero.height * 0.8);
    }

    // this.physics.world.createDebugGraphic();

    // ESCALA Y FLIP - Una sola vez y consistente
    const baseScale = this.isMobile ? 0.035 : 0.035;
    this.ptero.setScale(baseScale, baseScale); // Mantener proporción

    // VOLTEAR HORIZONTALMENTE para que mire hacia adelante
    this.ptero.setFlipX(true);
    ...
    this.physics.add.overlap(this.ptero, this.pipes, (ptero, pipe) =>
      this.handleCollision(
        ptero as Phaser.Physics.Arcade.Sprite,
        pipe as Phaser.Physics.Arcade.Sprite
      )
    );
  ...
  private createGameUI(): void {
    ...
    this.perfectText = this.add
      .text(this.gameWidth / 2, this.isMobile ? 40 : 50, "", {
  ...
  private addPipes(): void {
    ...
    const topScale = topHeight / 300;
    topPipe.setScale(1, topScale);
    topPipe.setVelocityX(-this.currentSpeed);

    const topBody = topPipe.body as Phaser.Physics.Arcade.Body;
    if (topBody) {
      // ✅ HITBOX QUE COINCIDE CON LA ESCALA APLICADA
      // La textura es 300px, pero queremos que la hitbox sea exactamente topHeight
      topBody.setSize(this.CONFIG.OBSTACLES.WIDTH, 300); // Usar altura original de textura
      // Phaser ajustará automáticamente con la escala
    }
    ...
    // Calcular la escala necesaria
    const bottomScale = bottomHeight / 300;
    bottomPipe.setScale(1, bottomScale);
    bottomPipe.setVelocityX(-this.currentSpeed);

    const bottomBody = bottomPipe.body as Phaser.Physics.Arcade.Body;
    if (bottomBody) {
      // ✅ HITBOX QUE COINCIDE CON LA ESCALA APLICADA
      bottomBody.setSize(this.CONFIG.OBSTACLES.WIDTH, 300); // Usar altura original de textura
      // Phaser ajustará automáticamente con la escala
    }
  ...
  private collectPowerUp(
    _ptero: Phaser.Physics.Arcade.Sprite,
    powerUp: Phaser.Physics.Arcade.Sprite
  ): void {
    this.createCollectEffect(powerUp.x, powerUp.y);

    const typedPowerUp = powerUp as Phaser.Physics.Arcade.Sprite & {
      powerType?: string;
    };
    ...
    switch (typedPowerUp.powerType) {
      ...
      case "powerup_points": {
        ...
        const currentPackage = Math.floor(
          this.score / this.CONFIG.GAME.DINOPOINTS_INTERVAL
        );
        if (currentPackage > this.packagesGiven) {
          this.packagesGiven = currentPackage;

          window.dispatchEvent(
            new CustomEvent("flappyPointsEarned", {
              detail: {
        ...
        window.dispatchEvent(
          new CustomEvent("flappyScoreUpdate", {
  ...
  private updateDifficulty(): void {
    const level = Math.floor(
      this.score / this.CONFIG.GAME.SPEED_LEVEL_INTERVAL
    );

    const newSpeed = Math.min(
      this.CONFIG.OBSTACLES.BASE_SPEED +
        level * this.CONFIG.OBSTACLES.SPEED_INCREASE,
    ...
    if (newSpeed !== this.currentSpeed) {
      ...
      window.dispatchEvent(
        new CustomEvent("flappySpeedUpdate", {
  ...
  private handleCollision(
    _ptero: Phaser.Physics.Arcade.Sprite,
    _pipe: Phaser.Physics.Arcade.Sprite
  ): void {
  ...
  private gameOverFn(): void {
    ...
    const finalStats = {
      ...
      pointsEarned:
        Math.floor(this.score / this.CONFIG.GAME.DINOPOINTS_INTERVAL) *
        this.CONFIG.GAME.DINOPOINTS_AMOUNT,
      perfectPasses: this.perfectPasses,
      maxCombo: this.combo,
    };

    window.dispatchEvent(
      new CustomEvent("flappyGameOver", {
        detail: finalStats,
      })
    );
  }
  ...
  public resetGame(): void {
    ...
    // USAR LA MISMA ESCALA BASE - NO 1!
    const baseScale = this.isMobile ? 0.035 : 0.035;
    this.ptero.setScale(baseScale, baseScale);
    this.ptero.setFlipX(true); // Mantener orientación correcta
  ...
  update(): void {
    ...
    // ANIMACIÓN DE ALETEO MÁS SUTIL - Solo cambiar Y, mantener X
    const baseScale = this.isMobile ? 0.035 : 0.035;
    const scaleVariation =
      Math.sin(this.time.now * this.CONFIG.PTERO.FLAP_ANIMATION_SPEED) * 0.003; // MUY sutil

    // Solo cambiar la escala Y para efecto de aleteo
    this.ptero.setScale(baseScale, baseScale + scaleVariation);
